...
def aggregate_pyramid(X: sp.csr_matrix, grid_coords: np.ndarray,
                      ...
                      transform: str = None,
                      coords: np.ndarray = None) -> dict[int, dict]:
    """
    ...
    grid_coords: integer grid coordinates (n_spots, 2) — (row, col) convention
    ...
    coords     : raw pixel coordinates from load_visium() (n_spots, 2)
                 [pxl_col_in_fullres, pxl_row_in_fullres] — if provided, stores
                 exact H&E pixel coords for each tissue pixel at every level
    """
    ...
    # ------------------------------------------------------------------ #
    #  Level 0: attach raw H&E coords directly — no lookup needed         #
    #  coords is already aligned with X row-by-row from load_visium()     #
    # ------------------------------------------------------------------ #
    if coords is not None:
        # tissue_idx at level 0 are flat grid indices — map to original spot order
        # via flat_to_spot, then index coords directly
        W0           = levels[0]['grid_shape'][1]
        tissue0      = levels[0]['tissue_idx']
        spot_flat    = grid_coords[:, 0] * W0 + grid_coords[:, 1]
        flat_to_spot = np.full(spot_flat.max() + 1, -1, dtype=np.int32)
        flat_to_spot[spot_flat] = np.arange(len(spot_flat))
        orig_idx0    = flat_to_spot[tissue0]
        # coords[:, 0] = pxl_col_in_fullres, coords[:, 1] = pxl_row_in_fullres
        levels[0]['he_row'] = coords[orig_idx0, 1].astype(np.float32)
        levels[0]['he_col'] = coords[orig_idx0, 0].astype(np.float32)
        print(f"Level 0 H&E coords stored | n={len(tissue0):,}")
    ...
    for lvl, d in levels.items():
        ...
        # ------------------------------------------------------------------ #
        #  Levels 1+: lookup table maps coarse tissue_idx -> original spots   #
        #  Each coarse cell covers 2^lvl x 2^lvl fine pixels — use any       #
        #  constituent fine spot as the representative H&E coordinate         #
        # ------------------------------------------------------------------ #
        if coords is not None and lvl > 0:
            tissue   = d['tissue_idx']
            orig_idx = flat_to_spot[tissue]
            valid    = orig_idx >= 0
            he_row   = np.full(len(tissue), np.nan, dtype=np.float32)
            he_col   = np.full(len(tissue), np.nan, dtype=np.float32)
            he_row[valid] = coords[orig_idx[valid], 1].astype(np.float32)
            he_col[valid] = coords[orig_idx[valid], 0].astype(np.float32)
            d['he_row'] = he_row
            d['he_col'] = he_col
...
if __name__ == "__main__":
    ...
    levels              = build_pyramid_masks(M0, target_max=10_000, plot=True)
    levels              = aggregate_pyramid(X, grid_coords, levels, data_type='visium', coords=coords)

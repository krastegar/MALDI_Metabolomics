...
def main() -> None:
    ...
    # Save coordinate maps and preprocessing outputs only
    # ------------------------------------------------------------------

    print("Saving coordinate maps from preprocessing stage")

    H, W = maldi["grid_shape"]

    # MALDI registration-grid coordinates
    ...
    # Map into full-resolution H&E coordinate space
    dy_he, dx_he = he["downsample_factor"]

    coord_df["he_x_pre"] = coord_df["maldi_x"] * dx_he
    coord_df["he_y_pre"] = coord_df["maldi_y"] * dy_he

    # Save coordinate lookup table
    coord_df.to_csv(out / "pre_registration_coordinate_map.csv", index=False)
